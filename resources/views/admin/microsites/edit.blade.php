    ...
    @section('content')
        <div class="max-w-4xl mx-auto py-10 px-6">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-200 rounded-lg shadow-sm hover:bg-gray-300 transition-colors">
                    Kembali
                </a>
            </div>
            ...
            <div class="bg-white shadow-lg rounded-2xl p-8 border border-gray-100">
                <form action="{{ route('admin.microsites.update', $microsite->id) }}" method="POST">
                    ...
                    <div class="space-y-6">
                        <!-- Nama Microsite -->
                        <div>
                            <label for="shortlink" class="block text-sm font-semibold text-gray-700">Nama Kegiatan</label>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    ...
                                    <select name="bidang_id" id="bidang_id"
                                        ...
                                        @foreach($bidangWithSeksi as $bidang)
                                            <option value="{{ $bidang->id }}" 
                                                {{ old('bidang_id', $microsite->bidang_id ?? '') == $bidang->id ? 'selected' : '' }}>
                                                {{ $bidang->nama_bidang }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select name="users_id" id="users_id"
                                        ...
                                        @foreach($user as $seksi)
                                            <option value="{{ $seksi->id }}" 
                                                {{ old('users_id', $microsite->users_id ?? '') == $seksi->id ? 'selected' : '' }}>
                                                {{ $seksi->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <input type="text" name="tanggal" id="tanggal"
                                    ...
                                    value="{{ old('tanggal', $microsite->tanggal) }}" placeholder="misal 17 Agustus 1945" required>
                                @error('tanggal')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
